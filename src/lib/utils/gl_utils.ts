...
// ************************************************** //
const wgl = typeof WebGL2RenderingContext !== "undefined"
...
export function getProgramSetters(gl, program, definitions) {
	...
	for (let i = 0; i < attribCount; i++) {
		...
		if (attribs[info.name]) throw new Error(`Overwriting attribute setter ${info.name}`);
	...
	for (let i = 0; i < unifromCount; i++) {
		...
		if (uniforms[info.name]) throw new Error(`Overwriting uniform setter ${info.name}`);
		let lastData = null;
